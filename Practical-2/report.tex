...
Correct variable classification is essential for both correctness and
performance under PCT:
\begin{itemize}
  \item \textbf{Sliced output} --- \texttt{iteration\_grid(row,:)} is
        written by exactly one worker per iteration, satisfying the
        sliced-variable requirement and preventing race conditions.
  \item \textbf{Sliced input} --- \texttt{y\_range(row)} is read as a
        scalar per iteration; MATLAB extracts the relevant slice for each
        worker automatically.
  \item \textbf{Broadcast variables} --- \texttt{x\_range} and
        \texttt{max\_iterations} are read-only and transmitted once to all
        workers at pool initialisation, minimising repeated data transfer.
  \item \textbf{Temporary variables} --- \texttt{x0}, \texttt{y0},
        \texttt{x}, \texttt{y}, and \texttt{iteration} are loop-private;
        each worker maintains its own copy with no synchronisation required.
\end{itemize}

A local accumulator \texttt{row\_result} buffers the inner-loop output
before assigning to the sliced grid, avoiding repeated indexed writes across
the process boundary and improving cache locality per worker.
...
$O(N)$ complexity with respect to resolution.  The 8K image (33.18\,MP)
takes 30.10\,s, roughly $45\times$ longer than SVGA (0.48\,MP, 0.67\,s).
The near-linear relationship between megapixels and execution time indicates
that per-pixel computation cost is approximately uniform when averaged
across the image, despite the per-pixel variation inherent to the
...
pixels escape in fewer than 10 iterations.  Row-based distribution does not
account for this variation---rows intersecting the main cardioid and period-2
bulb are disproportionately expensive.  MATLAB's \texttt{parfor} mitigates
this through dynamic scheduling, dispatching rows to workers as they become
available rather than pre-assigning fixed blocks.  This contributes to the
better-than-naive speedups observed, but for maximum efficiency, a
finer-grained work-stealing scheduler operating at the pixel or tile level
would better balance the heterogeneous workload.
